...
int execute_assemble_command(const AssembleConfig& config) {
    try {
        ...
        if (assembly_content.empty() || assembly_content.find_first_not_of(" \t\n\r") == std::string::npos) {
        ...
        if (!config.map.empty()) {
            ...
            if (!map_file.is_open()) {
                std::cerr << "mipsim: warning: cannot create symbol map file: " << config.map << std::endl;
                ...
                if (labelMap.empty()) {
                    ...
                    for (const auto& [label, address] : labelMap) {
                        map_file << label << " " << std::hex << "0x" << address << std::dec << std::endl;
                    }
                }
                map_file.close();
            }
