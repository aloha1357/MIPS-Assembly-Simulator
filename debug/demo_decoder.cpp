...
int main() {
    ...
    TestCase tests[] = {
        ...
        {0x2109000A, "addi $t1, $t0, 10"},
        {0x8C080004, "lw $t0, 4($zero)"},
        {0xAC090008, "sw $t1, 8($zero)"},
        {0x08000010, "j 0x40"},
        {0x0000000C, "syscall"},
        {0x2109FFFF, "addi $t1, $t0, -1"}
    };
    
    Cpu cpu;
    
    for (const auto& test : tests) {
        std::cout << "Instruction: 0x" << std::hex << std::uppercase 
                  << std::setfill('0') << std::setw(8) << test.word;
        ...
        if (instruction) {
            ...
            if (instruction->getName() == "addi") {
                ...
                std::cout << "  Execution result: $t1 = " 
                          << std::dec << cpu.getRegisterFile().read(9) << "\n";
                cpu.reset();
            }
        } else {
