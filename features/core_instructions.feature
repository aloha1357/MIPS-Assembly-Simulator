...
Feature: Core arithmetic and memory instructions
  The simulator must reproduce the observable behaviour of the baseline R‑/I‑type instruction subset.
  ...
  Scenario: beq taken branch flushes delay slot and updates PC
    ...
    """
    beq $t0, $t1, label
    addi $v0, $zero, 0      # delay slot (should be flushed)
    label:
    addi $v0, $zero, 42
    """
