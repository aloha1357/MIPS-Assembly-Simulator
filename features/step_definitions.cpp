...
THEN("^data memory address (0x[0-9A-Fa-f]+) should equal (0x[0-9A-Fa-f]+)$")
{
    ...
    EXPECT_EQ(actual, expected) << "Memory at address " << addressStr << " mismatch";
}
...
THEN("^the GUI console should contain \"([^\"]+)\"$")
{
    ...
    EXPECT_TRUE(context->consoleOutput.find(expectedOutput) != std::string::npos)
        << "Console output should contain '" << expectedOutput << "' but was: '"
...
THEN("^register (.+) should contain (0x[0-9A-Fa-f]+)$")
{
    ...
    EXPECT_EQ(actual, expected) << "Register " << regName << " should contain " << expectedStr
...
THEN("^registers (.+) and (.+) should remain unchanged$")
{
    ...
    EXPECT_EQ(actual1, expected1) << "Register " << reg1 << " should remain 0x" << std::hex
                                  << expected1 << " but is now 0x" << actual1;
    EXPECT_EQ(actual2, expected2) << "Register " << reg2 << " should remain 0x" << std::hex
                                  << expected2 << " but is now 0x" << actual2;
}
...
THEN("^the total execution time should be less than (\\d+) microseconds$")
{
    ...
    auto duration = std::chrono::duration_cast<std::chrono::microseconds>(context->endTime -
                                                                          context->startTime);
...
THEN("^each instruction should average less than (\\d+) nanoseconds$")
{
    ...
    auto duration =
        std::chrono::duration_cast<std::chrono::nanoseconds>(context->endTime - context->startTime);
