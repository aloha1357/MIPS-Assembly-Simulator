...
bool ImGuiMipsSimulatorGUI::initialize() {
    ...
    m_window = SDL_CreateWindow("MIPS Assembly Simulator", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, 1280, 720, window_flags);
    if (m_window == nullptr) {
...
void ImGuiMipsSimulatorGUI::renderRegisterViewer() {
    if (!m_showRegisterViewer) return;
...
void ImGuiMipsSimulatorGUI::renderPipelineViewer() {
    if (!m_showPipelineViewer) return;
