...
int main(int argc, char** argv)
{
    ...
    for (size_t i = 0; i < allLines.size(); ++i)
    {
        ...
        if (colonPos != std::string::npos)
        {
            ...
            if (i + 1 < allLines.size())
            {
                ...
                if (nextLine.rfind(".word", 0) == 0 || nextLine.rfind(".byte", 0) == 0 ||
                    nextLine.rfind(".asciiz", 0) == 0)
                {
                    nextLineIsData = true;
                }
            }
    ...
    std::cout << "assembler-first-pass-instructionAddress=" << instructionAddress
              << " parsedInstructions=" << instrs.size() << "\n";
    std::cout << "instructions=" << instrs.size() << " dataDirectives=" << dataDirs.size()
    ...
    {
        std::cout << "data at " << d.address << " type=" << d.type << " words=" << d.words.size()
                  << " bytes=" << d.bytes.size();
        if (!d.words.empty())
        {
            std::cout << " values=";
