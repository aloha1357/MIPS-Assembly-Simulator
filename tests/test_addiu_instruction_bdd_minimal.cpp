...
TEST_F(ADDIUInstructionBDDTest, BasicImmediateAddition)
{
    ...
    uint32_t instruction_encoding =
        (0x09 << 26) | (rs_register << 21) | (rt_register << 16) | immediate;
    ...
    auto addiu_instruction =
        std::make_unique<mips::ADDIUInstruction>(rt_register, rs_register, immediate);
...
TEST_F(ADDIUInstructionBDDTest, NegativeImmediateAddition)
{
    ...
    const uint32_t expected_result =
        rs_value + static_cast<uint32_t>(static_cast<int32_t>(static_cast<int16_t>(immediate)));
