...
syscall
    ...
    try {
        ...
        while (!cpu->shouldTerminate() && step < 20) {
            ...
            if (step == 2) { // After loading 'H' and 'i' 
                uint32_t t0 = cpu->getRegisterFile().read(8);  // $t0
                uint32_t t1 = cpu->getRegisterFile().read(9);  // $t1
                std::cout << "Step " << step << ": $t0 = 0x" << std::hex << t0 
                          << ", $t1 = 0x" << std::hex << t1 << std::endl;
            }
            
            if (step == 3) { // After shift
                uint32_t t1 = cpu->getRegisterFile().read(9);  // $t1 
                std::cout << "Step " << step << ": After shift, $t1 = 0x" << std::hex << t1 << std::endl;
            }
            
            if (step == 4) { // After add
                uint32_t t0 = cpu->getRegisterFile().read(8);  // $t0
                std::cout << "Step " << step << ": After add, $t0 = 0x" << std::hex << t0 << std::endl;
            }
            
            if (step == 5) { // After store
                uint32_t memValue = cpu->getMemory().readWord(0x1000);
                std::cout << "Step " << step << ": Memory[0x1000] = 0x" << std::hex << memValue << std::endl;
                
                // Extract bytes
                char byte0 = (memValue >> 0) & 0xFF;
                char byte1 = (memValue >> 8) & 0xFF;
                std::cout << "  Byte 0: 0x" << std::hex << (int)(unsigned char)byte0 << " ('" << byte0 << "')" << std::endl;
                std::cout << "  Byte 1: 0x" << std::hex << (int)(unsigned char)byte1 << " ('" << byte1 << "')" << std::endl;
            }
