...
protected:
    ...
    int parseRegisterNumber(const std::string& assembly) {
        if (assembly.find("$t0") != std::string::npos) return 8;
        if (assembly.find("$t1") != std::string::npos) return 9;
        if (assembly.find("$t2") != std::string::npos) return 10;
        if (assembly.find("$s0") != std::string::npos) return 16;
        return 0;
    }
    
    void validateInstructionExecution(const std::string& assembly, 
                                      uint32_t initialRs, 
                                      uint32_t initialPC, 
                                      uint32_t expectedPC) {
        // 設定暫存器初始值
...
TEST_F(BGTZInstructionIntegrationTest, BGTZInstruction_AssemblerVariants_ShouldParseCorrectly) {
    ...
    for (const auto& syntax : syntaxVariants) {
        ...
        // 應該有分支行為 (PC != oldPC + 4)
        EXPECT_NE(cpu->getProgramCounter(), oldPC + 4) << "Should branch for positive value with: " << syntax;
    }
}
...
TEST_F(BGTZInstructionIntegrationTest, BGTZInstruction_AssemblerInvalidSyntax_ShouldReturnEmpty) {
    ...
    for (const auto& syntax : invalidSyntax) {
        ...
        EXPECT_TRUE(instructions.empty()) 
            << "Should reject invalid syntax: " << syntax;
    }
}
...
TEST_F(BGTZInstructionIntegrationTest, BGTZInstruction_BranchConditionIntegration_ShouldBeAccurate) {
    ...
    for (const auto& test : branchTests) {
        ...
        EXPECT_EQ(cpu->getProgramCounter(), expectedPC) 
            << "Branch condition failed for " << test.description 
            << " (value=" << test.regValue << ")";
    }
}
