...
TEST_F(CLIAssembleCommandBDD, AssemblesSimpleProgramSuccessfully) {
    ...
    given_assembly_file("simple.asm", 
        "addi $t0, $zero, 42\n"
        "li $v0, 1\n"
        "move $a0, $t0\n"
        "syscall\n"
...
TEST_F(CLIAssembleCommandBDD, GeneratesSymbolMapWhenRequested) {
    ...
    given_assembly_file("with_labels.asm",
        "main:\n"
        "    addi $t0, $zero, 42\n"
        "    beq $t0, $zero, end\n"
        "    nop\n"
...
protected:
    ...
    void then_assemble_config_should_have(const std::string& input, 
...
TEST_F(CLIAssembleArgumentParsingBDD, ParsesAssembleCommandWithAllOptions) {
    ...
    when_parsing_args({"mipsim", "assemble", "program.asm", "-o", "output.bin", "--map", "symbols.map"});
