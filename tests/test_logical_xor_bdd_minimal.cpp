/**
 * @file test_logical_xor_bdd_minimal.cpp
 * @brief BDD Minimal XOR Instruction Test - Phase A
 * 
 * 按照BDD最小增量原則：
...
/**
 * @brief BDD XOR指令最小測試類別
 * 
 * 專注於單一 Scenario：XOR 指令執行位元 XOR 運算
 */
class LogicalXorMinimalBddTest : public ::testing::Test {
protected:
    void SetUp() override {
    ...
    void thenRegisterShouldContain(const std::string& regName, uint32_t expected) {
        ...
        EXPECT_EQ(actual, expected) 
            << "Register " << regName << " should contain 0x" << std::hex << expected 
...
// ============================================================================
// BDD Scenario 1: XOR 指令執行位元 XOR 運算 - 交替位元模式
// ============================================================================

/**
 * @brief BDD Test Case 1.1: XOR指令交替位元模式
 * 
 * Scenario: XOR 指令執行位元 XOR 運算
 *   Given 暫存器 $t0 包含 0xAAAAAAAA
 *   And 暫存器 $t1 包含 0x55555555  
 *   When 我執行指令 "xor $t2, $t0, $t1"
 *   Then 暫存器 $t2 應該包含 0xFFFFFFFF
 *   And 暫存器 $t0 應該仍然包含 0xAAAAAAAA
 *   And 暫存器 $t1 應該仍然包含 0x55555555
 *   And 指令應該在恰好 1 個 CPU 週期內完成
 */
TEST_F(LogicalXorMinimalBddTest, XorInstruction_AlternatingBitPattern_PhaseA) {
    // Given 暫存器 $t0 包含 0xAAAAAAAA
    givenRegisterContains("$t0", 0xAAAAAAAA);
    
    // And 暫存器 $t1 包含 0x55555555
    givenRegisterContains("$t1", 0x55555555);
    
    // When 我執行指令 "xor $t2, $t0, $t1"
    // Phase A: 這裡會失敗，這是預期的
    whenIExecuteInstruction("xor $t2, $t0, $t1");
    
    // Then 暫存器 $t2 應該包含 0xFFFFFFFF (AAAA ^ 5555 = FFFF)
...
/**
 * @brief BDD Test Case 1.2: XOR指令自身運算
 * 
 * 啟用第二個測試案例
 */
TEST_F(LogicalXorMinimalBddTest, XorInstruction_SelfXor_PhaseB) {
    // Given 暫存器 $s0 包含 0x12345678
    givenRegisterContains("$s0", 0x12345678);
    
    // And 暫存器 $s1 包含 0x12345678 (相同值)
    givenRegisterContains("$s1", 0x12345678);
    
    // When 我執行指令 "xor $s2, $s0, $s1"
    whenIExecuteInstruction("xor $s2, $s0, $s1");
    
    // Then 暫存器 $s2 應該包含 0x00000000 (A XOR A = 0)
    thenRegisterShouldContain("$s2", 0x00000000);
    
    // And 暫存器 $s0 應該仍然包含 0x12345678
    thenRegisterShouldRemainUnchanged("$s0");
    
    // And 暫存器 $s1 應該仍然包含 0x12345678
