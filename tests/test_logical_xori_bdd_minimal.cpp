/**
 * @file test_logical_xori_bdd_minimal.cpp
 * @brief XORI指令的BDD測試場景 - 嚴格BDD循環
 * 
 * Phase A: 撰寫階段 (紅燈) - 故意失敗的測試
 * 
 * 根據開發報告Phase 2要求：
 * - 立即值邏輯指令群組 (ORI ✅, ANDI ✅, XORI ⏳)
 * - 完成最後一個立即值邏輯指令，達成群組100%完成
 */
...
/**
 * @brief XORI指令BDD測試fixture
 * 
 * 使用Given-When-Then結構實現BDD場景
 */
class XoriInstructionBDDTest : public ::testing::Test {
protected:
    ...
    void then_register_should_equal(const std::string& reg, uint32_t expected) {
        ...
        EXPECT_EQ(actual, expected) 
            << "暫存器 " << reg << " 期望值: 0x" << std::hex << expected 
...
/**
 * @brief Scenario 1: XORI基本立即值XOR操作
 * 
 * Given register $t0 contains 0xF0F0F0F0
 * When  I execute "xori $t1, $t0, 0x0F0F"  
 * Then  register $t1 should contain 0xF0F0FFFF
 */
TEST_F(XoriInstructionBDDTest, Xori_BasicImmediateOperation_ShouldXorCorrectly) {
    // Given: 暫存器包含測試值
    given_register_contains("$t0", 0xF0F0F0F0);
    
    // When: 執行XORI指令
    when_program_executed_for_cycles("xori $t1, $t0, 0x0F0F", 1);
    
    // Then: 結果應該是XOR操作後的值 (零擴展立即值)
    then_register_should_equal("$t1", 0xF0F0FFFF);
    
    // 移除故意失敗，進入綠燈階段
    // FAIL() << "Phase A: 故意失敗 - 需要實作XORI指令的解碼器和組譯器支援";
}

/**
 * @brief Scenario 2: XORI切換位元操作
 * 
 * Given register $t0 contains 0x12345678
 * When  I execute "xori $t1, $t0, 0xFFFF"
 * Then  register $t1 should contain 0x1234A987 (低16位反轉)
 */
TEST_F(XoriInstructionBDDTest, Xori_BitToggleOperation_ShouldToggleCorrectly) {
    // Given: 暫存器包含測試值
    given_register_contains("$t0", 0x12345678);
    
    // When: 執行XORI指令進行位元切換
    when_program_executed_for_cycles("xori $t1, $t0, 0xFFFF", 1);
    
    // Then: 應該切換低16位元 (0x5678 XOR 0xFFFF = 0xA987)
    then_register_should_equal("$t1", 0x1234A987);
    
    // 移除故意失敗，進入綠燈階段
    // FAIL() << "Phase A: 故意失敗 - 需要實作XORI指令位元切換邏輯";
}
 ...
 */
TEST_F(XoriInstructionBDDTest, Framework_BddHelpers_ShouldWork) {
    // 測試Given helper
