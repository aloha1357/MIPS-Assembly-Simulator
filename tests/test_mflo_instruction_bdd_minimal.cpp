...
TEST_F(MFLOInstructionBDD, BasicLORead_ShouldMoveLOToRegister) {
    ...
    std::unique_ptr<Instruction> instruction = std::make_unique<MFLOInstruction>(8); // 目標到$t0 (暫存器8)
...
TEST_F(MFLOInstructionBDD, ZeroLORead_ShouldMoveZeroToRegister) {
    ...
    std::unique_ptr<Instruction> instruction = std::make_unique<MFLOInstruction>(9); // 目標到$t1 (暫存器9)
...
TEST_F(MFLOInstructionBDD, MaxValueLORead_ShouldMoveMaxValueToRegister) {
    ...
    std::unique_ptr<Instruction> instruction = std::make_unique<MFLOInstruction>(2); // 目標到$v0 (暫存器2)
...
TEST_F(MFLOInstructionBDD, DifferentTargetRegister_ShouldMoveOnlyToSpecifiedRegister) {
    ...
    std::unique_ptr<Instruction> instructionT2 = std::make_unique<MFLOInstruction>(10); // 目標到$t2 (暫存器10)
