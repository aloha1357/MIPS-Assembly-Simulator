 ...
 */
TEST_F(MissingInstructionsIntegrationTest,
       Given_AllMissingInstructions_When_ExecutedInSequence_Then_WorkTogether)
{
 ...
 */
TEST_F(MissingInstructionsIntegrationTest,
       Given_AllMissingOpcodes_When_Decoded_Then_CorrectInstructionsCreated)
{
 ...
 */
TEST_F(MissingInstructionsIntegrationTest,
       Given_ExistingInstructions_When_ExecutedWithNewOnes_Then_NoRegression)
{
    ...
    llo.execute(*cpu);
    EXPECT_EQ(0x00001234,
              cpu->getRegisterFile().read(8));  // Should preserve upper bits (which are 0)
}
