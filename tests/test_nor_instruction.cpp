/**
 * @file test_nor_instruction.cpp
 * @brief NOR instruction integration tests
 *
 * 按照BDD方法論實作的NOR指令整合測試，確保：
 * 1. InstructionDecoder 正確解碼 NOR 指令
 * 2. Assembler 正確解析 "nor" 語法
 */
...
class NorInstructionIntegrationTest : public ::testing::Test
{
  protected:
    void SetUp() override
    {
 ...
 *
 * Scenario: InstructionDecoder 支援 NOR 指令解碼
 *   Given 我有一個 InstructionDecoder
 *   When 我解碼 NOR 指令的機器碼 (function code 0x27)
 *   Then 解碼器應該返回 NorInstruction 物件
 *   And 指令名稱應該是 "nor"
 */
 ...
 *
 * Scenario: Assembler 支援 "nor" 語法解析
 *   Given 我有一個 Assembler
 *   When 我解析 "nor $t2, $t0, $t1" 指令
 *   Then 組譯器應該成功解析指令
 *   And 解析出的指令名稱應該是 "nor"
 */
